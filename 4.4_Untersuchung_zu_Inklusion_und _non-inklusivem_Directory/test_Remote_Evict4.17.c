/*used in Diagrams 4.17 of the thesis usage: gcc test_Remote_Evict4_17.c  -o test_remote_evict && GOMP_CPU_AFFINITY "0" ./test_remote_evict * and/or GOMP_CPU_AFFINITY "23" ./test_remote_evict*/
#define _GNU_SOURCE
#include <sched.h>
#include <sys/ioctl.h>
#include <stdio.h>
...
#define RUNS_EXTERNAL 5
#define RUNS_INTERNAL 5
...
static inline void mov(unsigned long long *ptr){
    asm volatile(
        "mov (%rdi), %r15;"
    );
}
...
void MessureRemote(unsigned long long ** AdressSameIndex,unsigned long long *start){
  ...
  {
    double avg=0,avgSt=0;
    double minSt=10000000;
    double minEvict=10000000;
    long long st,stop;
    int thread=omp_get_thread_num();
    for(int i=0;i<RUNS_EXTERNAL;i++){
      for(int j=0;j<RUNS_INTERNAL;j++){
          if(thread==0){adr=(rand()%(NUMBOFADR))+1;};
          long long lat=0;
	  #pragma omp barrier
	  ...
	  //usleep(1000);
          if(thread==0){
		for(int i=1;i<10;i++){
		mov(AdressSameIndex[adr]);}
		asm volatile("mfence");//creates a memory barrier (serializes loads and stores accordingly)
                rdtscll(st);
                    mov(AdressSameIndex[adr]);
                asm volatile("lfence");
                rdtscll(stop);
		if((stop-st)<minSt)minSt=stop-st;
	   };
          #pragma omp barrier
          if(thread==1){for(int p=0;p<NUMBOFADR;p++){if(p!=adr)mov(AdressSameIndex[p]);}}
          #pragma omp barrier
         // usleep(1000);
          #pragma omp barrier
         if(thread==0){
		st=0;
		stop=0; 
		int b=adr; //if index adr is not in L1D Cache we messure an Overhead
		asm volatile("mfence");//creates a memory barrier (serializes loads and stores accordingly)
                rdtscll(st);
                    //a=*(AdressSameIndex[g]+i);//access the whole cacheline
                //a=(a%100)+1;
                ...
                rdtscll(stop);
		//printf("%lld div\n",((stop-st)-lat));
                if((stop-st)<minEvict)minEvict=stop-st;};
          #pragma omp barrier
          //usleep(100);
          #pragma omp barrier
      }
      if(thread==0){avg+=minEvict;avgSt+=minSt;};
      if(thread==0){
		//printf("%lld,%lld\n",minEvict,minSt);
		minEvict=1000000;minSt=1000000;};
      //printf("%lld, ",min);
      #pragma omp barrier
      //usleep(100);
      #pragma omp barrier
    }
    if(thread==0){printf("%lf ",(avg/RUNS_EXTERNAL));fflush(stdout);};
